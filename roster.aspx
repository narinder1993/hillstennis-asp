        ...
        <script runat="server">
            protected void  Page_Load(object sender, EventArgs e)
            {
                ...
                if (con.State == System.Data.ConnectionState.Closed)
                    con.Open();
                System.Data.SqlClient.SqlCommand cmd = new System.Data.SqlClient.SqlCommand("Select COUNT(distinct teamNo) from teamInfo ", con);
                StringBuilder cstext2 = new StringBuilder();
                cstext2.Append("<script type=\"text/javascript\"> var newInput; function OnLoad() {");
                cstext2.Append("document.getElementById('roster').innerHTML ='';");
                int teams = (int) cmd.ExecuteScalar();
                Int16 i;
                for (i = 1; i <= teams; i++)
                {
                    int players;
                    System.Data.SqlClient.SqlCommand cmd1 = new System.Data.SqlClient.SqlCommand("Select COUNT(id) from teamInfo where teamNo="+i+"", con);
                    players = (int) cmd1.ExecuteScalar();                    
                    cstext2.Append("newInput=\"<br/><br/><center><table style='text-align:center'>");
                    cstext2.Append("<tr><td colspan='3' style='  font-size: xx-large; height: 40px;'>Team "+i+"</td></tr>");
                    cstext2.Append("<tr style='font-size: large;  height: 24px; background-color:#86c8b4; color:White'><th>Player Name</th><th>Gender</th><th>Player Rating</th></tr>");
                    for (int j = 1; j <= players; j++)
                    {
                        System.Data.SqlClient.SqlCommand name1 = new System.Data.SqlClient.SqlCommand(";with A AS ( SELECT ROW_NUMBER() OVER (ORDER BY Id desc) AS 'RN', first_name, gender, playerRating, IsCaptain from teamInfo where teamNo='"+i+"')select first_name from A where RN="+j+" ", con);
                        System.Data.SqlClient.SqlCommand gender1 = new System.Data.SqlClient.SqlCommand(";with A AS ( SELECT ROW_NUMBER() OVER (ORDER BY Id desc) AS 'RN', first_name, gender, playerRating, IsCaptain from teamInfo where teamNo='" + i + "')select gender from A where RN=" + j + " ", con);
                        System.Data.SqlClient.SqlCommand playerRating1 = new System.Data.SqlClient.SqlCommand(";with A AS ( SELECT ROW_NUMBER() OVER (ORDER BY Id desc) AS 'RN', first_name, gender, playerRating, IsCaptain from teamInfo where teamNo='" + i + "')select playerRating from A where RN=" + j + " ", con);
                        System.Data.SqlClient.SqlCommand captain1 = new System.Data.SqlClient.SqlCommand(";with A AS ( SELECT ROW_NUMBER() OVER (ORDER BY Id desc) AS 'RN', first_name, gender, playerRating, IsCaptain from teamInfo where teamNo='" + i + "')select IsCaptain from A where RN=" + j + " ", con);
                        string name = (string)name1.ExecuteScalar();
                        string gender = (string)gender1.ExecuteScalar();
                        double playerRating = (double)playerRating1.ExecuteScalar();
                        string captain = (string)captain1.ExecuteScalar();
                        if (captain.Equals("true") || captain.Equals("True"))
                        {
                            cstext2.Append("<tr style='  font-size: large;  height: 24px;'><td><b>" + name + "</b></td><td><b>" + gender + "</b></td><td><b>" + playerRating + "</b></td></tr>");
                        }
                        else
                        {
                            cstext2.Append("<tr style='  font-size: large;  height: 24px;'><td>" + name + "</td><td>" + gender + "</td><td>" + playerRating + "</td></tr>");
                        }
                    }
                    cstext2.Append("</table></center>\";");
                    cstext2.Append("document.getElementById('roster').innerHTML += newInput;");
                }
                cstext2.Append("}<");
            cstext2.Append("/script>");
            Page.ClientScript.RegisterClientScriptBlock(this.GetType(), cstext2.ToString(), cstext2.ToString());
            }
        </script>
        <style type="text/css">
nav ul {
						...
						<div class="menu_block">
							<nav class="horizontal-nav full-width horizontalNav-notprocessed">
								<ul class="sf-menu">
									...
									<li class="current"><a href="teamTennis.aspx">Team Tennis</a>
                                      <ul style="margin-left:-40px">
...
<!--==============================Content=================================-->
                ...
                <div id="roster">
                   <center> <input type="button" onclick="OnLoad()" class="btn" value="Click here To see Rosters"></center></center>
                </div>
            <br /><br />
